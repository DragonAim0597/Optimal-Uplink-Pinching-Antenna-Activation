%% Fig. 3(a): Average Sum-Rate versus Transmit Power
...
checkpointEvery = 50;
transmitPowerDbm = -20:5:10;      % Equal transmit power of every user
...
regionLengthX = 30;               % Service-region length in meters
...
cfg.checkpointFile = fullfile(outputDirectory, 'fig3a_checkpoint.mat');
cfg.resultFile = fullfile(outputDirectory, ...
    'fig3a_sum_rate_transmit_power.mat');
cfg.figureBaseName = fullfile(outputDirectory, ...
    'Fig3a_Sum_Rate_vs_Transmit_Power');
...
rng(seed, 'twister');
numPowerPoints = numel(transmitPowerDbm);
numBeams = numel(beamWidths);
noisePowerW = 10^((noisePowerDbm - 30) / 10);
transmitPowerW = 10.^((transmitPowerDbm - 30) / 10);
powerNoiseRatios = transmitPowerW / noisePowerW;
sumFixed = zeros(1, numPowerPoints);
sumGs = zeros(1, numPowerPoints);
sumBs = zeros(numBeams, numPowerPoints);
sumBnb = zeros(1, numPowerPoints);
sumVisited = zeros(1, numPowerPoints);
sumSqFixed = zeros(1, numPowerPoints);
sumSqGs = zeros(1, numPowerPoints);
sumSqBs = zeros(numBeams, numPowerPoints);
sumSqBnb = zeros(1, numPowerPoints);
startTrial = 1;
...
for trial = startTrial:numTrials
    [candidateChannels, fixedChannel] = generate_channels_local(cfg);

    for powerIndex = 1:numPowerPoints
        powerNoiseRatio = powerNoiseRatios(powerIndex);
        fixedRate = sum_rate_local(fixedChannel, powerNoiseRatio);
        sumFixed(powerIndex) = sumFixed(powerIndex) + fixedRate;
        sumSqFixed(powerIndex) = sumSqFixed(powerIndex) + fixedRate^2;

        [~, gsRate] = greedy_search_local(candidateChannels, powerNoiseRatio);
        sumGs(powerIndex) = sumGs(powerIndex) + gsRate;
        sumSqGs(powerIndex) = sumSqGs(powerIndex) + gsRate^2;

        for beamIndex = 1:numBeams
            [~, bsRate] = beam_search_local(candidateChannels, ...
                powerNoiseRatio, beamWidths(beamIndex));
            sumBs(beamIndex, powerIndex) = ...
                sumBs(beamIndex, powerIndex) + bsRate;
            sumSqBs(beamIndex, powerIndex) = ...
                sumSqBs(beamIndex, powerIndex) + bsRate^2;
        end

        [~, bnbRate, visitedNodes] = ...
            bnb_search_local(candidateChannels, powerNoiseRatio);
        sumBnb(powerIndex) = sumBnb(powerIndex) + bnbRate;
        sumSqBnb(powerIndex) = sumSqBnb(powerIndex) + bnbRate^2;
        sumVisited(powerIndex) = sumVisited(powerIndex) + visitedNodes;
    end
...
plot(transmitPowerDbm, results.fixed, styles{1}, 'Color', colors(1, :), ...
    'LineWidth', 1.8, 'MarkerSize', 7, 'MarkerFaceColor', 'w');
plot(transmitPowerDbm, results.gs, styles{2}, 'Color', colors(2, :), ...
    'LineWidth', 1.8, 'MarkerSize', 7, 'MarkerFaceColor', 'w');
for beamIndex = 1:numBeams
    styleIndex = 1 + mod(beamIndex + 1, numel(styles));
    plot(transmitPowerDbm, results.bs(beamIndex, :), styles{styleIndex}, ...
...
plot(transmitPowerDbm, results.bnb, styles{bnbStyleIndex}, ...
...
xlim([transmitPowerDbm(1), transmitPowerDbm(end)]);
